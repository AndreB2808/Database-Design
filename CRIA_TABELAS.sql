...
/*
-- WARNING: When removing comments of the block bellow all tables will be dropped

declare
   procedure drop_tables is
        cursor cur_drop is
            select 'drop table '|| object_name || ' cascade constraints purge' std
            from user_objects
            where object_type = 'TABLE'
            and trunc(created) >= '01/01/2025';
         statment varchar(4000);
         begin
            for reg_drop in cur_drop loop
                statment := reg_drop.std;
                execute immediate statment;
                dbms_output.put_line(statment);
            end loop;
            
            if statment is null then
...
Prompt ******  Errors are expected if such tables do not exist.
...
drop table emergency_contacts cascade constraints purge;
...
Prompt ******  Creating EMERGENCY_CONTACTS table ....

CREATE  TABLE emergency_contacts  (
    contact_id   NUMBER CONSTRAINT emergency_contacts_pk PRIMARY KEY,
    ...
    CONSTRAINT EMERGENCY_CONTACTS_FK FOREIGN KEY (employee_id) 
...
Prompt ******  Populating EMERGENCY CONTACTS table ....

INSERT INTO emergency_contacts (contact_id, employee_id, first_name, last_name, relationship, home_phone, work_phone, cell_phone )
VALUES ( 1,
          100,
         'Mary',
         'Bailey',
...
begin
   ...
   into v_count
   from REGIONS;
   dbms_output.put_line('******  Inserted '||v_count|| ' Lines on table REGIONS');
   select count(*) 
   into v_count
   from COUNTRIES;
   dbms_output.put_line('******  Inserted '||v_count|| ' Lines on table COUNTRIES');
   select count(*) 
   into v_count
   from LOCATIONS;
   dbms_output.put_line('******  Inserted '||v_count|| ' Lines on table LOCATIONS');
   select count(*) 
   into v_count
   from DEPARTMENTS;
   dbms_output.put_line('******  Inserted '||v_count|| ' Lines on table DEPARTMENTS');
   select count(*) 
   into v_count
   from JOBS;
   dbms_output.put_line('******  Inserted '||v_count|| ' Lines on table JOBS');   
   select count(*) 
   into v_count
   from EMPLOYEES;
   dbms_output.put_line('******  Inserted '||v_count|| ' Lines on table EMPLOYEES');
   select count(*) 
   into v_count
   from JOB_HISTORY;
   dbms_output.put_line('******  Inserted '||v_count|| ' Lines on table JOB_HISTORY');
   select count(*) 
   into v_count
   from EMERGENCY_CONTACTS;
   dbms_output.put_line('******  Inserted '||v_count|| ' Lines on table EMERGENCY_CONTACTS');
   select count(*) 
   into v_count
   from JOB_GRADES;
   dbms_output.put_line('******  Inserted '||v_count|| ' Lines on table JOB_GRADES');   
   ...
   if v_count <> 0 then 
     dbms_output.put_line('****** Objects Invalid Status Before Recompile: '||v_count);
     dbms_output.put_line('****** Compiling invalid objects ....');  
   else
      dbms_output.put_line('****** There are no objects with invalid status ....');  
   end if;
   :bcount := v_count;
end;
/
...
begin
   if v_count <> 0 then
      ...
      from USER_OBJECTS
      where status = 'INVALID';
     dbms_output.put_line('****** Objects Invalid Status After Recompile: '||v_count);   
     dbms_output.put_line('Objects Invalid List');
     for reg_invalid_obj in (select object_type,object_name
                             from USER_OBJECTS
                             where status = 'INVALID') loop      
         dbms_output.put_line(reg_invalid_obj.object_type|| ' ' || 
                              reg_invalid_obj.object_name);
     end loop;           
   end if;
   dbms_output.new_line;
   dbms_output.put_line('Finish execution in: '||to_char(sysdate,'dd/mm/rrrr hh24:mi:ss'));
end;
/
COMMIT;
Prompt 
